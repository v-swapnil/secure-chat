...
export class SecureCryptoEngine {
    ...
    async initializeSession(partnerId: string, partnerPublicKey: string): Promise<void> {
        ...
        const session = createSessionState(
            ...
            sharedSecret,
            ratchetKeyPair,
            null // Both parties start with null receiving ratchet key - it's sent in the first message!
        )
    ...
    async encryptMessage(message: string, partnerId: string): Promise<string> {
        ...
        // Don't perform DH ratchet on send - only on receive!
        // The ratchet happens when we receive a message with a new key
        ...
        console.log("encryptKey >>> ", encryptKey.slice(0, 8))
    ...
    async decryptMessage(encryptedEnvelope: string, partnerId: string): Promise<string> {
        ...
        // Skip if we already have this exact key stored (it's not a "new" ratchet)
        const needsRatchet = session.receivingRatchetPublicKey &&
            !arrayEquals(receivedRatchetKey, session.receivingRatchetPublicKey)

        if (needsRatchet) {
            // Partner has ratcheted, update our session
            // First, perform DH with our current key and their new key to get receiving chain
            ...
            // Generate new ephemeral key for our next send
            const newSendingRatchetKey = nacl.box.keyPair()

            // Perform second DH with our new key and their new key to get sending chain
            const dhOutput2 = dh(newSendingRatchetKey.secretKey, receivedRatchetKey)
            const { newRootKey: finalRootKey, newChainKey: newSendingChainKey } = ratchetRootKey(
                newRootKey,
                dhOutput2
            )
        ...
        } else if (!session.receivingRatchetPublicKey) {
            // First message from initiator - just store their ratchet key, don't ratchet yet
            session = {
                ...session,
                receivingRatchetPublicKey: receivedRatchetKey,
            }
        ...
        // Ratchet receiving chain to match message index
        let messageKey: Uint8Array | undefined

        while (session.receivingChainKey.index <= envelope.messageIndex) {
            const currentIndex = session.receivingChainKey.index
            const { messageKey: key, newSession } = getNextReceiveMessageKey(session)

            if (currentIndex === envelope.messageIndex) {
            ...
            // Not the right key yet, keep advancing
            session = newSession
        }

        if (!messageKey) {
            throw new Error(`Could not get message key for index ${envelope.messageIndex}`)
        }
        ...
        console.log("decryptKey >>> ", encryptKey.slice(0, 8))
